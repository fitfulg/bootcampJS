...
class Reserva {
    ...
    subtotalCalc(tipoHabitacion, desayuno, pax, noches) {
        this._subtotal = reservas.reduce(
            ...
            acc + noches * (this.roomType(tipoHabitacion) + this.additionalCharges(pax) + this.hasBreakfast(desayuno) * pax),
            0);
    }
    ...
    totalCalc(tipoHabitacion, desayuno, pax, noches) {
        ...
        this._total = reservas.reduce(
            (acc, { tipoHabitacion, desayuno, pax, noches }) =>
            acc + noches * (this.roomType(tipoHabitacion) + this.additionalCharges(pax) + this.hasBreakfast(desayuno) * pax) * IVA,
            0);
    }
